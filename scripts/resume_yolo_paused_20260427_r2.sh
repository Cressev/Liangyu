...
SCHED_LOG="$LOGDIR/resume_scheduler_${STAMP}.log"
...
run_resume() {
  ...
  {
    printf '[%s] START gpu=%s exp=%s ckpt=%s\n' "$(date '+%F %T')" "$physical_gpu" "$exp_name" "$ckpt"
    CUDA_VISIBLE_DEVICES="$physical_gpu" "$PY" trainMM.py --model "$ckpt" --resume --device 0
    rc=$?
    printf '[%s] END rc=%s gpu=%s exp=%s\n' "$(date '+%F %T')" "$rc" "$physical_gpu" "$exp_name"
    exit "$rc"
  } >>"$log" 2>&1
}

{
  ...
  printf 'using physical GPUs 6 and 7 only\n'
} >>"$SCHED_LOG"

(
  run_resume 6 yolov12s-rgb-bs64-fullmetrics run_yolo_bs64_fullmetrics_20260427_r2/yolov12s-rgb-bs64-fullmetrics/weights/last.pt
...
(
  run_resume 7 yolov12s-sonar-bs64-fullmetrics run_yolo_bs64_fullmetrics_20260427_r2/yolov12s-sonar-bs64-fullmetrics/weights/last.pt
