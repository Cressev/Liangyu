...
def visualize_x_to_3ch(x: torch.Tensor, colorize: bool = False, x_modality: str = 'depth') -> torch.Tensor:
    ...
    if x.shape[1] == 1:
        ...
        # Xch > 3 或 2通道: 不允许静默截断，显式报错
        raise ValueError(
            f"X模态有 {x.shape[1]} 个通道，无法自动转换为3通道用于可视化。"
            f"请预先选择/降维到1或3通道，或指定显式的通道选择策略。"
        )
...
def resolve_x_modality(modality_param: Optional[str], data: Optional[dict]) -> str:
    ...
    if modality_param:
        resolved = resolve_requested_modality_token(modality_param, data, default_x=x_modality)
        if resolved and not is_rgb_modality_token(resolved):
            return resolved

    if x_modality != 'unknown':
        return x_modality

    LOGGER.warning("无法解析X模态类型，使用默认值: unknown")
    return 'unknown'
