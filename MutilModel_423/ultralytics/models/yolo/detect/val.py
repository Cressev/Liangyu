...
class DetectionValidator(BaseValidator):
    ...
    def update_metrics(self, preds: List[Dict[str, torch.Tensor]], batch: Dict[str, Any]) -> None:
        ...
        for si, pred in enumerate(preds):
            ...
            self._target_areas.append(self._target_bbox_areas(pbatch))
            self._pred_gt_areas.append(self._matched_target_areas(predn, pbatch, stat, no_pred))
            # Evaluate
            if self.args.plots:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        size_maps = self._compute_size_maps()
        self.metrics.process(save_dir=self.save_dir, plot=self.args.plots, on_plot=self.on_plot)
        self.metrics.clear_stats()
        results = self.metrics.results_dict
        results.update(
            {
                "metrics/mAP50-95(S)": size_maps[0],
                "metrics/mAP50-95(M)": size_maps[1],
                "metrics/mAP50-95(L)": size_maps[2],
            }
        )
        self._cached_size_maps = size_maps
        return results

    def print_results(self) -> None:
        """Print training/validation set metrics per class."""
        size_maps = getattr(self, "_cached_size_maps", (0.0, 0.0, 0.0))
        pf = "%22s" + "%11i" * 2 + "%11.3g" * (len(self.metrics.keys) + 3)  # print format
        LOGGER.info(
            pf % ("all", self.seen, self.metrics.nt_per_class.sum(), *self.metrics.mean_results(), *size_maps)
        )
    ...
    def _target_bbox_areas(self, batch: Dict[str, Any]) -> np.ndarray:
        """Return native-pixel target bbox areas for COCO small/medium/large buckets."""
        if len(batch["bboxes"]) == 0:
            return np.zeros(0, dtype=np.float32)
        bboxes = batch["bboxes"]
        areas = ((bboxes[:, 2] - bboxes[:, 0]).clamp(0) * (bboxes[:, 3] - bboxes[:, 1]).clamp(0))
        return areas.cpu().numpy().astype(np.float32)

    def _matched_target_areas(
        self, preds: Dict[str, torch.Tensor], batch: Dict[str, Any], stat: Dict[str, np.ndarray], no_pred: bool
    ) -> np.ndarray:
        """Assign each prediction the area of its matched GT at IoU>=0.5; unmatched predictions are -1."""
        n_pred = len(preds["cls"])
        if no_pred:
            return np.zeros(0, dtype=np.float32)
        pred_area = np.full(n_pred, -1.0, dtype=np.float32)
        if len(batch["cls"]) == 0:
            return pred_area
        areas = self._target_bbox_areas(batch)
        if len(areas) == 0:
            return pred_area
        iou = box_iou(batch["bboxes"], preds["bboxes"])
        correct_class = batch["cls"][:, None] == preds["cls"]
        iou_cc = (iou * correct_class).cpu().numpy()
        matches = np.array(np.nonzero(iou_cc >= 0.5)).T
        if matches.shape[0]:
    ...
    def _compute_size_maps(self) -> Tuple[float, float, float]:
        """Compute mAP50-95 for COCO small/medium/large object area buckets."""
        ...
        unmatched = all_pred_areas < 0

        maps = []
        for lo, hi in ((0, 32**2), (32**2, 96**2), (96**2, float("inf"))):
            gt_mask = (all_gt_areas >= lo) & (all_gt_areas < hi)
            if gt_mask.sum() == 0:
                maps.append(0.0)
                continue
            pred_mask = ((all_pred_areas >= lo) & (all_pred_areas < hi)) | unmatched
            if pred_mask.sum() == 0:
                maps.append(0.0)
                continue
            res = ap_per_class(tp[pred_mask], conf[pred_mask], pred_cls[pred_mask], target_cls[gt_mask], plot=False)
            ap = res[5]
            maps.append(float(ap.mean()) if ap.size else 0.0)
        return tuple(maps)
    ...
    def plot_val_samples(self, batch: Dict[str, Any], ni: int) -> None:
        ...
        plot_images(
            ...
            cls=batch["cls"].squeeze(-1) if batch["cls"].ndim > 1 else batch["cls"],
            bboxes=batch["bboxes"],
            paths=batch["im_file"],
    ...
    def plot_predictions(
        ...
        plot_images(
            images=batch["img"],
            batch_idx=batched_preds["batch_idx"],
            cls=batched_preds["cls"],
            bboxes=batched_preds["bboxes"],
            confs=batched_preds.get("conf", None),
            paths=batch["im_file"],
