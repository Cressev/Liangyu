...
class VisualizationResult:
    ...
    def save(self, output_dir: Union[str, Path], prefix: str = "") -> List[str]:
        ...
        def _imwrite_rgb(path: Path, img: np.ndarray) -> None:
            arr = _to_uint8(img)
            # 约定：可视化输出统一为 RGB；保存时转换为 OpenCV 期望的 BGR
...
class HeatmapResult(VisualizationResult):
    ...
    def __init__(self,
        ...
        # 注意：data 使用 dict 保存 original/heatmap/overlay，便于统一保存与调试
        super().__init__(vis_type='heatmap', data=data, metadata=metadata)
        
    @property
    def primary_heatmap(self) -> Optional[np.ndarray]:
        """Get primary (RGB) heatmap if available."""
        if isinstance(self.heatmap, dict):
            return self.heatmap.get('primary') or self.heatmap.get('rgb')
        return self.heatmap if not isinstance(self.original_image, dict) else None

    # Backward-compatible alias
    @property
    def rgb_heatmap(self) -> Optional[np.ndarray]:
        """Get RGB heatmap if available (alias for primary_heatmap)."""
        return self.primary_heatmap

    @property
    def secondary_heatmap(self) -> Optional[np.ndarray]:
        """Get secondary (X) modality heatmap if available."""
        if isinstance(self.heatmap, dict):
            return self.heatmap.get('secondary') or self.heatmap.get('x')
        return None

    # Backward-compatible alias
    @property
    def x_heatmap(self) -> Optional[np.ndarray]:
        """Get X modality heatmap if available (alias for secondary_heatmap)."""
        return self.secondary_heatmap
...
class VisualizationManager:
    ...
    def _validate_input(self, source: Any) -> None:
        ...
        if isinstance(source, dict):
            ...
            # Check for valid modality keys（扩展 IR 同义词）
            valid_keys = {'primary', 'secondary', 'fused', 'rgb', 'x', 'thermal', 'depth', 'ir', 'infrared'}
            ...
            for idx, layer in enumerate(layers):
                ...
                if cache_key in self.cache:
                    ...
                    if save and cached_result:
                        ...
                        LOGGER.debug(f"Saved cached visualization results for layer {layer}: {len(saved_files)} files")
                ...
                try:
                    ...
                    # Fail‑Fast：不做占位降级，直接抛错，提示未实现/未安装
                    raise RuntimeError(
                ...
                try:
                    ...
                    if save and result:
                        ...
                        LOGGER.debug(f"Saved visualization results for layer {layer}: {len(saved_files)} files")
    ...
    def _placeholder_visualization(self,
        ...
        """
        LOGGER.debug(f"Creating placeholder visualization for method: {method}")
        LOGGER.debug(f"Target layers: {layers}")
        if kwargs:
        ...
        if isinstance(images, dict):
            ...
            for modal, img in images.items():
                ...
                heatmap = cv2.applyColorMap(gradient, cv2.COLORMAP_TURBO)  # OpenCV returns BGR
