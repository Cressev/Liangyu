...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            # Warmup channels should match the model's expected input channels.
            # NOTE: For multimodal models, data.yaml may keep 'channels: 3' while the dataloader emits (3 + Xch) channels.
            data_channels = int(self.data.get("channels", 3))
            model_channels = int(getattr(model, "ch", data_channels))
            warmup_channels = model_channels or data_channels
            if warmup_channels != data_channels:
                LOGGER.warning(
                    f"Warmup通道数使用模型channels={warmup_channels} (data.yaml channels={data_channels})。"
                    f"请确保数据加载输出通道与模型输入通道一致。"
                )
            model.warmup(imgsz=(1 if pt else self.args.batch, warmup_channels, imgsz, imgsz))  # warmup
    ...
    def check_stats(self, stats):
        """Check statistics for validation."""
        pass
