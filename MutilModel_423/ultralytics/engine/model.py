...
from ultralytics.utils.torch_utils import (
    compute_model_gflops,
    is_multimodal_complexity_model,
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        if not self.predictor:
            if is_multimodal_complexity_model(self.model):
            ...
            try:
                imgsz = int(getattr(self.predictor.args, "imgsz", 640))
                if maybe_log_multimodal_canonical_summary(self.model, imgsz=imgsz, logger=LOGGER):
                    setattr(self.predictor, "_default_complexity_logged", True)
            except Exception:
                pass
        else:
        ...
        if is_cli:
            ...
            # 非CLI模式
            if is_multimodal_predictor:
                return self.predictor(rgb_source=rgb_source, x_source=x_source, stream=stream, strict_match=strict_match, speed=speed)
            else:
                # 标准predictor只接受source参数
