...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    # 从默认配置读取多模态项目版本（single source: default.yaml）
    MM_PROJECT_VERSION = DEFAULT_CFG_DICT.get("mm_project_version")
    if not MM_PROJECT_VERSION:
        MM_PROJECT_VERSION = "1.0.0"
    s = (
        f"Ultralytics {__version__} 🚀 Python-{PYTHON_VERSION} torch-{torch.__version__} "
        f"| 多模态 {MM_PROJECT_VERSION} "
    )
...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    if not verbose:
        return
    if should_suppress_legacy_multimodal_complexity_summary(model):
        return
...
MM_COMPLEXITY_OWNER_PENDING = "_mm_complexity_owner_pending"
MM_COMPLEXITY_OWNER_EMITTED = "_mm_complexity_owner_emitted"
MM_COMPLEXITY_DIRECT_INGRESS = "_mm_complexity_direct_ingress"
MM_COMPLEXITY_OWNER_PENDING_PUBLIC = "mm_complexity_owner_pending"
MM_COMPLEXITY_OWNER_EMITTED_PUBLIC = "mm_complexity_owner_emitted"
MM_COMPLEXITY_DIRECT_INGRESS_PUBLIC = "mm_complexity_direct_ingress"


def _complexity_base_model(model):
    """Return the underlying PyTorch model that should be inspected for multimodal complexity ownership."""
    if model is None:
        return None
    model = de_parallel(model)
    inner = getattr(model, "model", None)
    if getattr(model, "pt", False) and isinstance(inner, torch.nn.Module):
        return de_parallel(inner)
    return model


def _yaml_has_multimodal_routes(yaml_cfg) -> bool:
    """Return True if the YAML-like config contains multimodal route markers."""
...
def is_multimodal_complexity_model(model) -> bool:
    """Best-effort detection of whether a model belongs to the multimodal family."""
    base = _complexity_base_model(model)
    if base is None:
        return False
    if getattr(base, "is_multimodal", False):
        return True
    if getattr(base, "mm_router", None) is not None or getattr(base, "multimodal_router", None) is not None:
        return True
    if getattr(base, "x_modality_name", None) not in (None, "", "unknown"):
        return True
    metadata = getattr(base, "metadata", None)
    if isinstance(metadata, dict) and metadata.get("x_modality_name") not in (None, "", "unknown"):
        return True
    yaml_cfg = getattr(base, "yaml", None)
    if _yaml_has_multimodal_routes(yaml_cfg):
        return True
    if isinstance(yaml_cfg, dict):
        channels = yaml_cfg.get("channels", yaml_cfg.get("ch"))
        if isinstance(channels, int) and channels > 3:
            return True
    return False


def set_multimodal_complexity_owner_state(model, pending=None, emitted=None, direct_ingress=None) -> None:
    """Propagate multimodal complexity ownership flags to both wrapper and underlying model objects."""
    targets = []
    ...
    for target in targets:
        if pending is not None:
            setattr(target, MM_COMPLEXITY_OWNER_PENDING, bool(pending))
            setattr(target, MM_COMPLEXITY_OWNER_PENDING_PUBLIC, bool(pending))
        if emitted is not None:
            setattr(target, MM_COMPLEXITY_OWNER_EMITTED, bool(emitted))
            setattr(target, MM_COMPLEXITY_OWNER_EMITTED_PUBLIC, bool(emitted))
        if direct_ingress is not None:
            setattr(target, MM_COMPLEXITY_DIRECT_INGRESS, bool(direct_ingress))
            setattr(target, MM_COMPLEXITY_DIRECT_INGRESS_PUBLIC, bool(direct_ingress))


def get_multimodal_complexity_owner_state(model) -> dict:
    """Return owner-handshake flags from wrapper/base model."""
    state = {
        "pending": False,
        "emitted": False,
        "direct_ingress": False,
    }
    for target in (model, _complexity_base_model(model)):
        if target is None:
            continue
        state["pending"] = state["pending"] or bool(
            getattr(target, MM_COMPLEXITY_OWNER_PENDING, False)
            or getattr(target, MM_COMPLEXITY_OWNER_PENDING_PUBLIC, False)
        )
        state["emitted"] = state["emitted"] or bool(
            getattr(target, MM_COMPLEXITY_OWNER_EMITTED, False)
            or getattr(target, MM_COMPLEXITY_OWNER_EMITTED_PUBLIC, False)
        )
        state["direct_ingress"] = state["direct_ingress"] or bool(
            getattr(target, MM_COMPLEXITY_DIRECT_INGRESS, False)
            or getattr(target, MM_COMPLEXITY_DIRECT_INGRESS_PUBLIC, False)
        )
    return state


def should_suppress_legacy_multimodal_complexity_summary(model) -> bool:
    """Return True when legacy model summary logging should be suppressed for multimodal models."""
    return is_multimodal_complexity_model(model)


def maybe_log_multimodal_canonical_summary(model, imgsz=640, logger=None, direct_ingress=False) -> bool:
    """Log the canonical multimodal complexity summary exactly once for the current call chain."""
    if not is_multimodal_complexity_model(model):
        return False
    state = get_multimodal_complexity_owner_state(model)
    should_log = state["pending"] or direct_ingress or state["direct_ingress"]
    if state["emitted"] or not should_log:
        return False

    from ultralytics.nn.mm.complexity import compute_default_multimodal_complexity_report, log_default_complexity

    try:
        report = compute_default_multimodal_complexity_report(_complexity_base_model(model), imgsz=int(imgsz))
        log_default_complexity(_complexity_base_model(model), report, logger or LOGGER)
        set_multimodal_complexity_owner_state(model, pending=False, emitted=True, direct_ingress=False)
        return True
    except Exception as e:
        (logger or LOGGER).warning(f"Skipping multimodal complexity summary due to error: {e}")
        set_multimodal_complexity_owner_state(model, pending=False, emitted=True, direct_ingress=False)
        return False
...
def format_param_count(n: int) -> str:
    """Return a compact human-readable parameter count."""
    if n >= 1e9:
        return f"{n / 1e9:.2f}B"
    if n >= 1e6:
        return f"{n / 1e6:.2f}M"
...
def log_multimodal_model_complexity(model, imgsz=640, modality: str | None = None) -> None:
    """Log parameter count plus multimodal architecture/runtime GFLOPs in a unified format."""
    params = get_num_params(model)
    route_tag = modality or "dual"
    arch_gflops = compute_model_gflops(model, imgsz=imgsz, modality=None, route_aware=False)
    route_gflops = compute_model_gflops(model, imgsz=imgsz, modality=modality, route_aware=True)
    LOGGER.info(
        f"Params: {format_param_count(params)} ({params:,}) | "
        f"GFLOPs (arch): {arch_gflops:.2f} | GFLOPs (route[{route_tag}]): {route_gflops:.2f}"
    )
...
def compute_model_gflops(model, imgsz=640, modality: str | None = None, route_aware: bool = True) -> float:
    ...
    - route_aware=True: 保持模型当前的多模态路由设置，若提供 modality 且模型包含 mm_router，则在拷贝上设置运行时模态，
      使用实际 imgsz 做一次 thop.profile，得到更贴近“当前模态/路由”的计算量。
    - route_aware=False: 不干预路由设置，仅以模型结构为准（架构级），使用实际 imgsz 做一次 profile。

    Args:
        model (nn.Module): 模型
        imgsz (int | list): 输入尺寸（方形或[h, w]）
        modality (str | None): 运行时模态，例如 'rgb'、'ir'、'thermal' 等；None 表示双模态（Dual）
        route_aware (bool): 是否进行“路由感知”的 GFLOPs 评估
    ...
    try:
        ...
        LOGGER.warning("GFLOPs 统计需要安装 'ultralytics-thop'，已跳过（返回0.0）")
        return 0.0

    m = de_parallel(model)
    try:
        mcopy = deepcopy(m)
        # 路由感知封装：在 forward 前设置运行时模态，确保 forward 内部创建/使用的路由得到正确参数
        if route_aware:
            class _RouteWrapper(torch.nn.Module):
                def __init__(self, base, modality_):
                    super().__init__()
                    self.base = base
                    self.modality = modality_
                def forward(self, x):
                    try:
                        if hasattr(self.base, 'mm_router') and self.base.mm_router is not None:
                            self.base.mm_router.set_runtime_params(self.modality)
                        elif hasattr(self.base, 'model') and hasattr(self.base.model, 'mm_router') and self.base.model.mm_router is not None:
                            self.base.model.mm_router.set_runtime_params(self.modality)
                    except Exception:
                        pass
                    return self.base(x)
            mcopy = _RouteWrapper(mcopy, modality)
        # 规范 imgsz
        if not isinstance(imgsz, (list, tuple)):
            imgsz = [int(imgsz), int(imgsz)]
        p = next(mcopy.parameters())
        c = int(p.shape[1])
        device = p.device
        im = torch.empty((1, c, imgsz[0], imgsz[1]), device=device)
        flops = thop.profile(mcopy, inputs=[im], verbose=False)[0] / 1e9 * 2
        return float(flops)
    except Exception as e:
        LOGGER.warning(f"GFLOPs 计算失败: {e}")
        return 0.0
