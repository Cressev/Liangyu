...
class TaskAlignedAssigner(nn.Module):
    ...
    Attributes:
        ...
        topk2 (int): Secondary topk value for STAL (Selective Top-K Aligned Learning) filtering.
        num_classes (int): The number of object classes.
        alpha (float): The alpha parameter for the classification component of the task-aligned metric.
        beta (float): The beta parameter for the localization component of the task-aligned metric.
        stride (list): List of stride values for different feature levels (for small object enhancement).
    ...
    def __init__(
        ...
        Args:
            ...
            stride (list, optional): List of stride values for small object enhancement. None to disable.
            topk2 (int, optional): Secondary topk for STAL filtering. None defaults to topk (no STAL).
        """
        super().__init__()
        self.topk = topk
        ...
        if self.topk2 > self.topk:
            raise ValueError(f"topk2 ({self.topk2}) must be <= topk ({self.topk})")
    ...
    def get_pos_mask(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt):
        ...
        """
        # Select candidates in ground truth boxes (with optional stride enhancement)
        if self.stride is not None:
            mask_in_gts = self._select_candidates_in_gts_with_stride(anc_points, gt_bboxes, mask_gt)
        else:
    ...
    def _select_candidates_in_gts_with_stride(self, xy_centers, gt_bboxes, mask_gt, eps=1e-9):
        """
        Select positive anchor centers with stride-based small object enhancement.

        For small objects (width or height < smallest stride), expand the bbox to increase anchor matching.
        ...
        """
        # Convert xyxy to xywh for size checking
        gt_bboxes_xywh = xyxy2xywh(gt_bboxes)
        # Check if w or h is smaller than the smallest stride
        wh_mask = gt_bboxes_xywh[..., 2:] < self.stride[0]
        # Expand small objects to stride[1] size for better anchor matching
        ...
        gt_bboxes_enhanced = xywh2xyxy(gt_bboxes_xywh)

        # Standard anchor selection with enhanced bboxes
        n_anchors = xy_centers.shape[0]
        bs, n_boxes, _ = gt_bboxes_enhanced.shape
        lt, rb = gt_bboxes_enhanced.view(-1, 1, 4).chunk(2, 2)
    ...
    def select_highest_overlaps(self, mask_pos, overlaps, n_max_boxes, align_metric):
        ...
        Includes STAL (Selective Top-K Aligned Learning) secondary filtering when topk2 != topk.

        Args:
            mask_pos (torch.Tensor): Positive mask, shape (b, n_max_boxes, h*w).
            overlaps (torch.Tensor): IoU overlaps, shape (b, n_max_boxes, h*w).
            n_max_boxes (int): Maximum number of ground truth boxes.
            align_metric (torch.Tensor): Alignment metric for STAL filtering.
        ...
        # STAL: Secondary topk filtering when topk2 != topk
        if self.topk2 != self.topk:
            align_metric_masked = align_metric * mask_pos
            max_overlaps_idx = torch.topk(align_metric_masked, self.topk2, dim=-1, largest=True).indices
...
def make_anchors(feats, strides, grid_cell_offset=0.5):
    ...
    for i, stride in enumerate(strides):
        h, w = feats[i].shape[2:] if isinstance(feats, (list, tuple)) else (int(feats[i][0]), int(feats[i][1]))
