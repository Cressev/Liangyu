...
from ultralytics.nn.mm.utils import MM_METADATA_KEY, MM_X_MODALITY_KEY, sync_x_modality_name_to_model
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, heads):  # includes all Detect/Segment/Pose/OBB subclasses (e.g., LSCD variants)
            ...
            def _forward(x):
                ...
                out = self.forward(x)
                # LSPCD 等新版头 training 时返回 dict，统一提取 feats
                if isinstance(out, dict):
                    if self.end2end and "one2many" in out:
                        out = out["one2many"]
                    if isinstance(out, dict):
                        return out.get("feats", out)
                    return out
                # 旧版头：seg_pose_obb 返回 tuple 取第一个元素
                seg_pose_obb = SEGMENT_CLASS + POSE_CLASS + OBB_CLASS
                return out[0] if isinstance(m, seg_pose_obb) else out
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        metadata = ckpt.get(MM_METADATA_KEY) if isinstance(ckpt.get(MM_METADATA_KEY), dict) else {}
        model_metadata = getattr(model, "metadata", None)
        if not isinstance(model_metadata, dict):
            model_metadata = {}
        model_metadata.update(metadata)
        model.metadata = model_metadata
        sync_x_modality_name_to_model(model, metadata.get(MM_X_MODALITY_KEY, "unknown"))
...
def parse_model(d, ch, verbose=True, dataset_config=None):
    ...
    for i, layer_config in enumerate(d["backbone"] + d["head"]):  # from, number, module, args, [input_type]
        ...
        # DEA 专用分支：允许旧风格参数并归一化到新签名
            ...
            c2 = c_main
        elif m is PointSMCAPriorBranch:
            if isinstance(f, (list, tuple)) and len(f) != 1:
                raise ValueError(f"{m.__name__} expects 1 input, got {f} at layer {i}")
            c_prior = args[0] if len(args) > 0 and args[0] is not None else 64
            c_prior = make_divisible(min(c_prior, max_channels) * width, 8)
            args = [c_prior, *args[1:]]
            c2 = c_prior
        elif m is PointSMCAPriorFusion:
            if isinstance(f, int) or len(f) != 2:
                raise ValueError(f"{m.__name__} expects 2 inputs (rgb_feat, prior_out), got {f} at layer {i}")
            c_rgb = ch[f[0]]
            c_prior = ch[f[1]]
            c_out = args[0] if len(args) > 0 and args[0] is not None else c_rgb
            if c_out != nc:
                c_out = make_divisible(min(c_out, max_channels) * width, 8)
            stage_idx = args[1] if len(args) > 1 else 1
            mode = args[2] if len(args) > 2 else "gated"
            args = [c_out, stage_idx, mode, c_rgb, c_prior]
            c2 = c_out
