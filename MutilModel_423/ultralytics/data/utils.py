...
ARRAY_FORMATS = {"npy", "npz"}  # numpy-backed image suffixes
MM_PRIMARY_FORMATS = IMG_FORMATS | ARRAY_FORMATS  # multimodal primary source formats (NPY/NPZ for array inputs)
...
def load_primary_array_file(im_file: Union[str, Path]) -> np.ndarray:
    """Load a primary multimodal array file (.npy/.npz) using the shared default-key policy."""
    im_file = Path(im_file)
    suffix = im_file.suffix.lower()
    if suffix == ".npy":
        return np.load(im_file, allow_pickle=False)
    if suffix == ".npz":
        with np.load(im_file, allow_pickle=False) as npz_file:
            ...
            if selected_key is None:
                if len(npz_file.files) == 1:
                    ...
                    raise ValueError(
                        f"npz file {im_file} contains multiple arrays {npz_file.files}, "
                        f"cannot determine default key. Use standard keys (image/arr_0)."
                    )
            return np.asarray(npz_file[selected_key])
    raise ValueError(f"Unsupported primary array suffix for {im_file}")


def validate_primary_array_shape(im_file: Union[str, Path], arr: np.ndarray) -> Tuple[int, int]:
    """Validate a primary array input and return its image-like HW shape."""
    if arr.ndim < 2:
        raise ValueError(f"npy file {im_file} has insufficient dimensions: {arr.shape}")
    shape = tuple(int(v) for v in arr.shape[:2])
    assert (shape[0] > 9) & (shape[1] > 9), f"image size {shape} <10 pixels"
    return shape


def filter_primary_array_cache_candidates(im_files: List[str], allowed_formats: set[str]) -> List[str]:
    """Skip same-stem .npy/.npz cache siblings when a standard image already exists beside them."""
    allowed = {fmt.lower() for fmt in allowed_formats}
    if not (allowed & ARRAY_FORMATS):
        return im_files

    standard_exts = IMG_FORMATS & allowed
    if not standard_exts:
        return im_files

    file_index = {(str(Path(f).parent), Path(f).stem, Path(f).suffix[1:].lower()) for f in im_files}
    filtered = []
    for f in im_files:
        p = Path(f)
        suffix = p.suffix[1:].lower()
        if suffix in ARRAY_FORMATS:
            sibling_key = (str(p.parent), p.stem)
            if any((sibling_key[0], sibling_key[1], ext) in file_index for ext in standard_exts):
                continue
        filtered.append(f)
    return filtered
...
def verify_image(args: Tuple) -> Tuple:
    ...
    nf, nc, msg = 0, 0, ""
    try:
        # NPY/NPZ 文件：跳过 PIL 校验，仅验证维度
        suffix = Path(im_file).suffix.lower()
        if suffix in ('.npy', '.npz'):
            arr = load_primary_array_file(im_file)
            validate_primary_array_shape(im_file, arr)
            nf = 1
...
def verify_image_label(args: Tuple) -> List:
    ...
    try:
        # Verify images
        # NPY/NPZ 文件：跳过 PIL 校验，仅验证维度
        im_suffix = Path(im_file).suffix.lower()
        if im_suffix in ('.npy', '.npz'):
            arr = load_primary_array_file(im_file)
            shape = validate_primary_array_shape(im_file, arr)
            nf = 1
        else:
            im = Image.open(im_file)
            im.verify()  # PIL verify
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> Dict[str, Any]:
    ...
    # Add Xch field support for multimodal X modality channel count
    data["Xch"] = data.get("Xch", 3)  # default to 3 channels for X modality
    if data["Xch"] not in [1, 3]:
        LOGGER.warning(f"Xch={data['Xch']} is not a common value (1 or 3). Please verify X modality channel count is correct.")

    # Validate: train/val/test should NOT be absolute paths
    for k in "train", "val", "test":
        if data.get(k):
            paths_to_check = [data[k]] if isinstance(data[k], str) else data[k]
            for p in paths_to_check:
                if Path(p).is_absolute():
                    raise ValueError(
                        f"\n{dataset} '{k}: {p}' 使用了绝对路径，不允许在此字段中使用绝对路径。\n"
                        f"请将绝对路径放在 'path' 字段，'{k}' 字段使用相对路径。\n"
                        f"正确写法示例:\n"
                        f"  path: {Path(p).parent.parent}  # 绝对根路径\n"
                        f"  {k}: {Path(p).name}  # 相对路径\n"
                    )
