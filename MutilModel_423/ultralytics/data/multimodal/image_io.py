...
DType: 本模块在加载时通过 normalize_modality_array() 自动归一化为 float32。
确保下游增强管线和模型只遇到 uint8（标准 RGB）和 float32（已归一化数据）两种 dtype。
"""
...
def normalize_modality_array(arr: np.ndarray, modality: str = "x") -> np.ndarray:
    """
    将模态 numpy 数组归一化为 float32 [0, 1] 范围。

    在数据加载的最早期调用，确保下游增强管线和模型只遇到 float32。
    自动处理所有常见 dtype，无需用户干预。

    Args:
        arr: HWC numpy 数组，任意数值 dtype
        modality: 模态标识 ('rgb' 或 'x')，用于日志消息
    ...
    # uint16 -> /65535 (16-bit 深度图/热红外典型场景)
    if dt == np.uint16:
        return arr.astype(np.float32) / 65535.0

    # float64 -> 直接 cast（精度损失在模型计算可接受范围内）
    if dt == np.float64:
        return arr.astype(np.float32)

    # int64 / int32 / int16 -> cast to float32, 按数据范围自动归一化
    if np.issubdtype(dt, np.integer):
        arr_f = arr.astype(np.float32)
        max_val = arr_f.max()
        if max_val > 1.0:
            arr_f = arr_f / max_val if max_val > 0 else arr_f
        return arr_f
...
class MultiModalImageIOMixin:
    ...
    def find_corresponding_x_image(
        ...
        Args:
            ...
            dataset_root: 数据集根目录（可选，避免依赖 parent 层级猜测）
            split_name: split 名称如 train/val/test（可选）
        ...
        if dataset_root is not None:
            ...
            split_dir = split_name or rgb_path.parent.name
        else:
            ds_root = rgb_path.parent.parent.parent
            split_dir = rgb_path.parent.name
        x_dir = ds_root / x_modality_dir / split_dir
