...
from ultralytics.data.utils import (
    ...
    IMG_FORMATS,
    check_file_speeds,
    filter_primary_array_cache_candidates,
    load_primary_array_file,
)
...
class BaseDataset(Dataset):
    ...
    def get_img_files(self, img_path: Union[str, List[str]], allowed_formats: Optional[set] = None) -> List[str]:
        """
        Read image files from the specified path.

        Args:
            img_path (str | List[str]): Path or list of paths to image directories or files.
            allowed_formats (set, optional): Set of allowed file extensions. Defaults to IMG_FORMATS.
        ...
        """
        try:
            allowed = allowed_formats or IMG_FORMATS
            f = []  # image files
            ...
            im_files = sorted(x.replace("/", os.sep) for x in f if x.rpartition(".")[-1].lower() in allowed)
            im_files = filter_primary_array_cache_candidates(im_files, allowed)
    ...
    def load_image(self, i: int, rect_mode: bool = True) -> Tuple[np.ndarray, Tuple[int, int], Tuple[int, int]]:
        ...
        if im is None:  # not cached in RAM
            ...
            if suffix in ('.npy', '.npz'):
                # Primary array file (multimodal NPY/NPZ)
                try:
                    im = load_primary_array_file(f)
                except Exception as e:
                    raise FileNotFoundError(f"Failed to load primary array file {f}: {e}") from e
            elif suffix in ('.tif', '.tiff'):
                # TIFF with original bit-depth
            ...
            # 高精度 dtype 归一化（npy/tiff 可能包含 uint16/float32/float64/int64）
            if suffix in ('.npy', '.npz', '.tif', '.tiff') and im is not None:
                if im.dtype != np.uint8:
                    from ultralytics.data.multimodal.image_io import normalize_modality_array
                    im = normalize_modality_array(im, modality="rgb")
    ...
    def cache_images(self, cache=None) -> None:
        """Cache images to memory or disk for faster training.

        Args:
            cache: Cache mode ('ram', 'disk', or None). If None, uses self.cache.
        """
        ...
        fcn, storage = (self.cache_images_to_disk, "Disk") if cache_mode == "disk" else (self.load_image, "RAM")
