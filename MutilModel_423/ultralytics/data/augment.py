...
def __safe_image_compression(quality_lower, quality_upper, p):
    """兼容不同 albumentations 版本的 ImageCompression 构建。"""
    import albumentations as _A
    try:
        return [_A.ImageCompression(quality_range=(quality_lower, quality_upper), p=p)]
    except TypeError:
        return [_A.ImageCompression(quality_lower=quality_lower, quality_upper=quality_upper, p=p)]
...
class BaseMixTransform:
    ...
    def __call__(self, labels: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # For multi-modal datasets, use __getitem__ to get properly processed images
        if hasattr(self.dataset, 'load_multimodal_image'):
            # Multi-modal dataset: use get_image_and_label then load multi-modal image
            mix_labels = []
            for i in indexes:
                label = self.dataset.get_image_and_label(i)
                # Replace RGB image with multi-modal image
                multimodal_img = self.dataset.load_multimodal_image(i)
                label["img"] = multimodal_img
                mix_labels.append(label)
        else:
            # Standard dataset: use original logic
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i in range(4):
            ...
            # Place img in img4
            if i == 0:  # top left
                fill_val = 114.0 if np.issubdtype(img.dtype, np.floating) else 114
                img4 = np.full((s * 2, s * 2, img.shape[2]), fill_val, dtype=img.dtype)  # base image with 4 tiles
    ...
    def __init__(self, p: float = 1.0) -> None:
        ...
        try:
            ...
            T = [
                ...
                A.RandomGamma(p=0.0),
                *__safe_image_compression(75, 100, 0.0),
            ]
