...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    train_gpu       = [int(x) for x in os.environ.get("FRCNN_GPUS", "0").split(",") if x != ""]
    #---------------------------------------------------------------------#
    #   fp16        是否使用混合精度训练
    ...
    #---------------------------------------------------------------------#
    classes_path    = os.environ.get("FRCNN_CLASSES_PATH", 'model_data/voc_classes.txt')
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = os.environ.get("FRCNN_MODEL_PATH", 'model_data/voc_weights_resnet.pth')
    #------------------------------------------------------#
    ...
    #                       (当Freeze_Train=False时失效)
    #------------------------------------------------------------------#
    Init_Epoch          = int(os.environ.get("FRCNN_INIT_EPOCH", "0"))
    Freeze_Epoch        = int(os.environ.get("FRCNN_FREEZE_EPOCH", "0"))
    Freeze_batch_size   = int(os.environ.get("FRCNN_FREEZE_BATCH", "4"))
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = int(os.environ.get("FRCNN_EPOCHS", "350"))
    Unfreeze_batch_size = int(os.environ.get("FRCNN_BATCH", "8"))
    #------------------------------------------------------------------#
    #   Freeze_Train    是否进行冻结训练
    #                   默认先冻结主干训练后解冻训练。
    #                   如果设置Freeze_Train=False，建议使用优化器为sgd
    #------------------------------------------------------------------#
    Freeze_Train        = os.environ.get("FRCNN_FREEZE_TRAIN", "False").lower() in {"1", "true", "yes", "y"}
    ...
    #------------------------------------------------------------------#
    save_period         = int(os.environ.get("FRCNN_SAVE_PERIOD", "10"))
    #------------------------------------------------------------------#
    #   save_dir        权值与日志文件保存的文件夹
    #------------------------------------------------------------------#
    save_dir            = os.environ.get("FRCNN_SAVE_DIR", 'logs')
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    eval_flag           = True
    eval_period         = int(os.environ.get("FRCNN_EVAL_PERIOD", "10"))
    #------------------------------------------------------------------#
    #   num_workers     用于设置是否使用多线程读取数据，1代表关闭多线程
    #                   开启后会加快数据读取速度，但是会占用更多内存
    #                   在IO为瓶颈的时候再开启多线程，即GPU运算速度远大于读取图片的速度。
    #------------------------------------------------------------------#
    num_workers         = int(os.environ.get("FRCNN_WORKERS", "4"))
    #----------------------------------------------------#
    #   获得图片路径和标签
    #----------------------------------------------------#
    train_annotation_path   = os.environ.get("FRCNN_TRAIN_TXT", '2007_train.txt')
    val_annotation_path     = os.environ.get("FRCNN_VAL_TXT", '2007_val.txt')
